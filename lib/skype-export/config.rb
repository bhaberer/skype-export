...
module SkypeExport
  ...
  module Config
    def self.location(skype_username:)
      return ENV['SKYPE_DB_LOCATION'] if ENV.key?('SKYPE_DB_LOCATION')
      ...
      when /mingw/
        if File.exist?("#{ENV['APPDATA']}\\Skype")
          ...
          format_windows_path get_default_skype_data_location_on_windows_8(skype_username)
        end
      when /linux/
    ...
    def self.get_default_skype_data_location_on_windows_8(skype_username)
      ...
      raise IOError, "The default Skype directory doesn't exist." unless skype_folder
